...
def latest_bar_key(asset_class: AssetClass | str, bar_size: str) -> str:
...
class MarketDataRedisClient:
    ...
    async def set_latest_bar(self, bar: OHLCVBar) -> str:
        await self.connect()
        key = latest_bar_key(bar.asset_class, bar.bar_size)
        await self._client.set(key, bar.to_redis_json())
        return key

    async def get_latest_bar(self, asset_class: AssetClass | str, bar_size: str) -> OHLCVBar | None:
        await self.connect()
        payload = await self._client.get(latest_bar_key(asset_class, bar_size))
        if payload is None:
            return None
